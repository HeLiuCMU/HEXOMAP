...
/*
    Crystallography func
*/
__global__ void euler_zxz_to_mat(float* afEuler,  // radians
                                 float* afMat,    // active rotation matrix
                                 int    iNAngle,  // # of angles
                                ){
...
__device__ void d_euler_zxz_to_mat(float* afEuler,  // radians
                                   float* afMat,    // active rotation matrix
                                ){
...
__global__ void mat_to_euler_ZXZ(float* afMatIn,     // active rotation matrix
                                 float* afEulerOut,  // radians
                                 int iNAngle,        // # of angles
                                ){
    /*
...
__global__ void rand_mat_neighb_from_euler(float* afEulerIn,  // sampling center
                                           float* afMatOut,   // output matrix
                                           float* afRand,     // input random numbers
                                           float  fBound,     // sampling bound
                                        ){
    ...
    */
    ...
    if(blockIdx.x>0){
        ...
        // keep the original input
        // append the orignal euler at the beginning of the array (in case it is the best solution)
        d_euler_zxz_to_mat(afEulerIn + threadIdx.x * 3, afMatOut + blockIdx.x * blockDim.x * 9 + threadIdx.x * 9);
    }
}

__global__ void misorien(float* afMisOrien, 
                         float* afM0, float* afM1, float* afSymM){
    /*
    ...
    * 
    * [TODO]
    * symmetry operator applied to both M0 and M1
    */
    int i = blockIdx.x*blockDim.x + threadIdx.x;
    float afTmp0[9];
    float afTmp1[9];
...
/*
    Forward simualtion
*/
__device__ bool GetScatteringOmegas( float &fOmegaRes1,  // omega_fidelpair_1, output
                                     float &fOmegaRes2,  // omega_fidelpair_2, output
                                     float &fTwoTheta,   // output
                                     float &fEta,        // output
                                     float &fChi,        // output  
                                     const float *aScatteringVec,  // input
                                     const float &fBeamEnergy,     // input, KeV
                                ){
    ////////////////////////test passed ////////////////////////
...
__device__ bool GetPeak(int &iJ1, int &iJ2,
                        int &iK1, int &iK2,
                        bool &bHit1,bool &bHit2,
...
__global__ void create_bin_expimages(char* acExpDetImages, const int* aiDetStartIdx,
	...
	/*
	 * create the image matrix (sparse matrix -> dense matrix)
...
__global__ void hitratio_multi_detector(const int iNVoxel,        // # of voxels
                                        const int iNOrientation,  // # of guessed orientations/grains
                                        const int iNG,            // # of g vectors
                                        const float* __restrict__ afDetInfo, 
                                        const int iNDet,          // # of detector distances
                                        const int iNRot,          // # of omegas
                                        const int* aiJ, const int* aiK,  // pixel pos
                                        const int* aiRotN,        // which omega(s) correspond to the peak
                                        const bool* abHit,        // within detector coverage
                                        float* afHitRatio,        // output, hit ratio
                                        int* aiPeakCnt,           // number of all diffraction Peaks hit on detector in the simulation
                                    ){
	/*
	 * 100x100 voxel takes 13.8123515625s
	 * This version using texture memory to storage tcExpData
     * this version optimize some detail, runs faster than 
     * hitratio_multi_detector_old_backup (deprecated), no error so far.
